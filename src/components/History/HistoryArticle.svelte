<script>
    ...
    let link_id=1;
    let target = null;
    $: targetRectX = target && target.getBoundingClientRect().left;
    $: targetRectY = target && target.getBoundingClientRect().top;
    $: targetWidth = target && target.width;
...
<article>  <!-- TODO: ㅈ같은 Footnote 팝업 작성 -->
    ...
    <br />
    <a href="#fn-2" bind:clientWidth={width} on:mouseenter={() => {onMouse()}} on:mouseleave={outMouse} class="annotation" bind:this={target}><span id="rfn-2">[2]</span></a>
    ...
    <span id="fn-1"><a href="#rfn-1" class="annotation">[1]</a> {annotation_content}</span>  <!-- below -->
